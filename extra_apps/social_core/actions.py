...
def do_complete(backend, login, user=None, redirect_name='next',
    ...
    response.set_cookies('name',user.name if user.name else user.username,max_age=24*3600)
    response.set_cookies('token', jwt_encode_handler(payload), max_age=24 * 3600)
    return response
